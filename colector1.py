...
def pesquisa(dork):
	...
	dorkAtual = open("dorkAtual1.txt", "w")
	...
	for i in range(0,1000,10):
		...
		for merda in links:
			...
			for urlFromGoogle in urls:
				...
				txtEmails = open("emailsDoGoogle1.txt", "a+")
				for email in emails:
					print("[+] Email -> [" + email + "]")
...
arquivoComAsDork = "dorks1.txt"
ips = []
arq = open(arquivoComAsDork, 'r')
todasAsDorks = arq.readlines()
arq.close()
mudaIPSocks5("127.0.0.1", 8081)
ipAtual = 0
quantidadeIP = len(ips)
criaTunelSSH("socks", ips[ipAtual], 8081)
